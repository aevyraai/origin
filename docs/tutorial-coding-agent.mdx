...
```
plan (planner)
  ├── search_docs       — canned reference for the algorithm
  └── check_signature   — schema-checks the proposed function signature
synthesize (synthesizer) — distils plan + docs into an implementation recipe
write_code (coder)       — first draft
run_tests                — exec'd in a subprocess, structured pass/fail
[ if any test fails: ]
  diagnose_failure (debugger) — reads test output, proposes a fix
  write_code (coder)          — revised draft (SAME prompt_id as first)
  run_tests                   — re-runs against the original test cases
respond (responder)      — final user-facing reply
```
